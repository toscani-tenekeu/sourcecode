...
// If it's an OPTIONS request (preflight), just return headers
...
function log_message($message) {
    $log_file = '../logs/subscription.log';
    $timestamp = date('Y-m-d H:i:s');
    file_put_contents($log_file, "[$timestamp] $message" . PHP_EOL, FILE_APPEND);
}

// Log the request
log_message("Subscription process started");
...
// Validate input data
if (!isset($data['plan_id']) || !isset($data['email'])) {
...
if ($plan_result->num_rows === 0) {
    log_message("Error: Plan not found (ID: $plan_id)");
...
$amount = isset($data['amount']) ? (int)$data['amount'] : (int)$plan['price'];
$email = $data['email'];
...
$external_id = 'SUB_' . date('mdHis') . rand(100, 999);

// Prepare the message for the payment
$message = "Subscription to " . $plan['name'] . " Seller Plan on SourceCode";

// Prepare the redirect URL
$base_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'];
$redirect_url = $base_url . "/seller/subscription-success";
...
try {
    ...
    if (isset($response['message']) && $response['message'] === 'Request successful') {
        // Store subscription information in session
        ...
        echo json_encode([
            ...
            'payment_link' => $response['link'] // Include payment link directly
        ]);
    } else {
        // Log the error
